...
public class Location {
    ...
    public static float distanceBetween(double lat1, double lon1, double lat2, double lon2/* , char unit */) {
        ...
        double dist = Math.sin(MathUtils.deg2rad(lat1)) * Math.sin(MathUtils.deg2rad(lat2))
                + Math.cos(MathUtils.deg2rad(lat1)) * Math.cos(MathUtils.deg2rad(lat2))
                        * Math.cos(MathUtils.deg2rad(theta));
        dist = Math.acos(dist);
        ...
        if (Float.isNaN(floatdist) || Float.isInfinite(floatdist)) {
            // Log.i("GeoUtils", "Result of distanceBetween NaN or infinite for lat1="+lat1+
