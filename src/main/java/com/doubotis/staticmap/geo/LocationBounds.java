...
public class LocationBounds {
	...
	/**
	 * Traduit ce BBOX en Well-Known-Text, afin d'?tre compris par une base
	 * Spatialite par exemple.
	 * 
	...
	public double getAreaKm2() {
		...
		double bboxwidth = Location.distanceBetween(minlat, minlon, minlat, maxlon) / 1000;
		double bboxheight = Location.distanceBetween(minlat, minlon, maxlat, minlon) / 1000;
	...
	/**
	 * Returns true if the <code>GLatLng</code> is contained inside this BBOX. False
	 * otherwise.
	 */
	 ...
	 * 
	 * @param inclusive Flag indicates that true must be returned only if the
	 *                  parameter BBOX is fully contained into this BBOX.
	 */
	...
	/**
	 * Creates a BBOX that wraps a specified LatLng, with an area in meters around
	 * the point.
	 * 
	...
	public static LocationBounds enveloppe(Location latLng, long distance) {
		...
		double mpdlon = Location.distanceBetween(location1[0], location1[1], location2[0], location2[1]);
		...
		double mpdlat = Location.distanceBetween(location1[0], location1[1], location2[0], location2[1]);
